...
**Stack:** Svelte 5 (runes) · SvelteKit 2.x library mode · TypeScript · Vite · `@keenmate/pure-admin-core` v2.9.0-rc14
...
- **Search entry points** — core (rc12) defines 3 patterns; we implement 2. **(1) `NavbarSearch`** = the `.pa-navbar-search` pill *trigger* that opens the palette (sizes `--xs…--xl`). **(2) `NavbarSearchField`** = the `.pa-navbar-search--field` inline *live search* — a real `<input>` + anchored `.pa-search-autocomplete` dropdown, Svelte-native (no core JS). One `globalSearch(query) → SearchResult[]` call; each row's optional `group` field (server "group column") is bucketed client-side into `.pa-search-autocomplete__section` groups (single round-trip, no fan-out); `SearchGroup` = presentation-only label/icon/order/`limit`. Renders core's exact `__item-icon`/`__item-content`(`__item-name`/`__item-type`)/`__item-badge` rows. **(3) `.pa-sidebar__search`** (sidebar trigger) is **not** wrapped yet. `CommandPalette` stays a separate Svelte-native engine — the field does not (yet) share it; unifying the palette engine across both surfaces is deferred to upstream work.
...
**Pure Admin Core:** 2.9.0-rc14 (synced) · **Svelte:** 5.x · **SvelteKit:** 2.x
