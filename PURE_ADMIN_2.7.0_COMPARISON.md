...
| Added | Text-contrast tiers: `--pa-text-strong` (85%) / `--pa-text-secondary` (70%) / `--pa-text-tertiary` (55%) — `color-mix(in srgb, var(--pa-text-color-1) X%, transparent)`, so they invert with theme mode. Emitted at `:root, .pa-mode-light, .pa-mode-dark` so each scope recomputes against its own `--pa-text-color-1`. | **None** — CSS-only. Document for app authors. |
